...
data_preds_sum = np.zeros(len(df_data))
n_models = 100

for seed in np.arange(1, n_models + 1, 1):
    ...
    final_dataset = TensorDataset(torch.from_numpy(df_data.to_numpy().astype(np.float32)))
    final_loader = DataLoader(final_dataset, batch_size=2000, shuffle=False)

    ANN_pred = torch.sigmoid(torch.tensor(mytools.test_final(test_loader, ANN, device))).numpy()
    ANN_pred_final = torch.sigmoid(torch.tensor(mytools.test_final(final_loader, ANN, device))).numpy()

    test_preds_sum += ANN_pred
    data_preds_sum += ANN_pred_final


# Assign ensemble-averaged predictions
df_test['ANN'] = test_preds_sum / n_models
df_test['InvM'] = mytools.get_InvM(df_test)

df_data['ANN'] = data_preds_sum / n_models
...
ax_top.axvline(x=ANN_selection, color='k', linestyle='--', label='selection line')
ax_top.legend()
...
plt.tight_layout()
if FakeGen:
    plt.savefig('/Users/mghrear/data/ML_data/2019_pass2/ensemble_results/plots/unscaled/data_mc_comparison/data_mc_comparison_FakeGen_ensemble_avg.png')
else:
    plt.savefig('/Users/mghrear/data/ML_data/2019_pass2/ensemble_results/plots/unscaled/data_mc_comparison/data_mc_comparison_ensemble_avg.png')
plt.close(fig)


# --- Data / MC comparison plot after quality cuts ---
...
plt.tight_layout()
if FakeGen:
    plt.savefig('/Users/mghrear/data/ML_data/2019_pass2/ensemble_results/plots/unscaled/data_mc_comparison/data_mc_comparison_QualCuts_FakeGen_ensemble_avg.png')
else:
    plt.savefig('/Users/mghrear/data/ML_data/2019_pass2/ensemble_results/plots/unscaled/data_mc_comparison/data_mc_comparison_QualCuts_ensemble_avg.png')
plt.close(fig)
...
if FakeGen:
    df_data_cut.to_pickle('/Users/mghrear/data/ML_data/2019_pass2/ensemble_results/inference/unscaled/2019_pass2_data_full_FakeGen_ensemble_avg.pk')
else:
    df_data_cut.to_pickle('/Users/mghrear/data/ML_data/2019_pass2/ensemble_results/inference/unscaled/2019_pass2_data_full_ensemble_avg.pk')
