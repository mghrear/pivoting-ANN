...
model_dir = '/Users/mghrear/data/ML_data/patch/ensemble_NHP1_QualCuts/'
...
for p in Path(model_dir).iterdir():
    ...
    plt.legend()
    plt.savefig(f'/Users/mghrear/Desktop/Ensemble_study/NHP1_QualCuts/MC_plots/ANN_ensemble_run{run_number}_mc_invmass.png')
...
out_dir = '/Users/mghrear/data/HPS_data/ensemble_NHP1_QualCuts/'
...
for index, row in df_ANN_selections.iterrows():
    ...
    ANN = mytools.Classifier(in_features=X_test.shape[1]).to(device)
    ANN.load_state_dict(torch.load( '/Users/mghrear/data/ML_data/patch/ensemble_NHP1_QualCuts/classifier_adv_2021_v9_pass5_run'+str(run_number)+'_limited.pt' , map_location=device))
    ANN.eval()
